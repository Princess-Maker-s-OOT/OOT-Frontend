...
export async function apiClient(
  ...
  if (requiresAuth) {
    ...
    console.log("[apiClient] 토큰 확인:", {
      ...
      tokenPreview: accessToken?.substring(0, 30) + "..." || "없음"
    })
    if (accessToken) {
  ...
  try {
    ...
    if (response.status === 401 && requiresAuth) {
      console.log("[apiClient] 401 에러 발생, 토큰 갱신 시도")
      const refreshed = await refreshAccessToken()
      if (refreshed) {
        console.log("[apiClient] 토큰 갱신 성공, 재시도")
        // 토큰 갱신 성공 시 재시도
        ...
        console.log("[apiClient] 토큰 갱신 실패, 로그인 페이지로 이동")
