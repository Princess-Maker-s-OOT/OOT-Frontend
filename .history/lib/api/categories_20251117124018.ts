...
export function buildCategoryTree(flatCategories: CategoryResponse[]): CategoryNode[] {
  ...
  // 아동 계층 구조 (ID 92-105)
  const childMid = sorted.filter(c => c.id >= 92 && c.id <= 96) // 중분류
  const childLeaf = sorted.filter(c => c.id >= 97 && c.id <= 105) // 소분류
  ...
  childMid.forEach((mid) => {
    ...
    const midIdRanges: Record<number, [number, number]> = {
      92: [97, 97],   // 아우터 (소분류 없음, 백엔드 데이터 확인 필요)
      93: [97, 101],  // 상의 → 티셔츠들
      94: [98, 103],  // 하의 → 바지들
      95: [99, 105],  // 신발 → 스니커즈들
      96: [100, 100], // 액세서리 (소분류 없음, 백엔드 데이터 확인 필요)
    }
