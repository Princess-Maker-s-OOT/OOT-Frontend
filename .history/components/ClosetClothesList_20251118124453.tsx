    ...
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {items.map((item) => (
        <div key={item.linkId} className="border rounded p-4 bg-white shadow-sm flex gap-4 items-center">
          {/* 옷 이미지 */}
          {item.imageUrl ? (
            <img src={item.imageUrl} alt="옷 이미지" className="w-20 h-20 object-cover rounded" />
          ) : (
          ...
          <div className="flex-1 space-y-1">
            ...
            <div className="text-sm text-gray-600">색상: {item.clothesColor} / 사이즈: {item.clothesSize}</div>
          </div>
          {/* 버튼 영역 */}
          {isMine && (
