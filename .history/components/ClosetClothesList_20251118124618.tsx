  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.linkId} className="border rounded p-4 bg-white shadow-sm flex gap-4 items-center">
            {/* 옷 이미지 */}
            {imageUrl ? (
              <img src={imageUrl} alt="옷 이미지" className="w-20 h-20 object-cover rounded" />
            ) : (
              <div className="w-20 h-20 bg-gray-100 flex items-center justify-center rounded text-gray-400 text-xs">이미지 없음</div>
            )}
            {/* 옷 정보 */}
            <div className="flex-1 space-y-1">
              <div className="text-base font-semibold">{detail?.description || item.description}</div>
              <div className="text-sm text-gray-600">색상: {detail?.clothesColor || item.clothesColor} / 사이즈: {detail?.clothesSize || item.clothesSize}</div>
            </div>
            {/* 버튼 영역 */}
            {isMine && (
              <div className="flex flex-col gap-2 items-end">
                <button
                  ...
                  className="bg-sky-600 text-white px-3 py-1 rounded hover:bg-sky-700 disabled:opacity-50 text-xs"
                >
                ...
                <button
                  ...
                  className="bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center hover:bg-red-600 disabled:opacity-50 text-xs"
                  title="옷 제거"
                >
