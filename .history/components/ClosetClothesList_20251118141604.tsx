    ...
    <div className="max-w-[80rem] mx-auto p-8">
      <div className="grid grid-cols-3 lg:grid-cols-4 gap-24">
        {items.map((item) => {
          ...
          return (
            <div key={item.linkId} className={`relative flex flex-col items-center justify-between border-2 border-pink-200 rounded-2xl bg-sky-50 shadow-lg p-8 w-72 h-96 transition hover:scale-105 hover:shadow-xl`}>
              {/* X 버튼 오른쪽 상단 */}
              {isMine && (
                <button
              ...
              {/* 옷 이미지 */}
              {imageUrl ? (
                <img src={imageUrl} alt="옷 이미지" className="w-40 h-40 object-cover rounded-2xl mb-6" />
              ) : (
                <div className="w-40 h-40 bg-gray-100 flex items-center justify-center rounded-2xl text-gray-400 text-base mb-6">이미지 없음</div>
              )}
              {/* 옷 정보 */}
              <div className="flex-1 flex flex-col items-center justify-center text-center w-full mb-6">
                <div className="text-xl font-bold mb-3 truncate max-w-[160px]">{detail?.description || item.description}</div>
                <div className="text-base text-gray-600">{detail?.clothesColor || item.clothesColor} / {detail?.clothesSize || item.clothesSize}</div>
              </div>
              {isMine && (
                <button
                  onClick={() => handleWearToday(item.clothesId)}
                  disabled={wearingId === item.clothesId}
                  className="w-11/12 py-4 bg-gradient-to-r from-pink-400 to-sky-400 text-white rounded-full hover:from-pink-500 hover:to-sky-500 font-bold shadow-lg text-lg transition mb-2"
