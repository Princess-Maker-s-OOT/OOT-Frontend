...
function SalePostsPageInner() {
  ...
  async function loadPosts() {
    try {
      ...
      // 제목 검색만 반영, 예외 처리: 빈 문자열, 공백 등
      const trimmedTitle = searchTitle.trim();
      const searchByTitle = trimmedTitle.length > 0;
      const salePostsRes = await apiGet<SalePostListResponse>(
        `/api/v1/sale-posts/public?page=0&size=20${searchByTitle ? `&title=${encodeURIComponent(trimmedTitle)}` : ""}${userLat !== null && userLng !== null ? `&lat=${userLat}&lng=${userLng}` : ""}`,
        { requiresAuth: false }
      );
