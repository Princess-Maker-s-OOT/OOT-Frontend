...
export default function ImageUploadAndSaveForm() {
  ...
  async function handleUploadAndSave(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const presigned = await createPresignedUrl(parsedPresigned.data)
      if (!("data" in presigned) || !presigned.data) {
        setError((presigned as any)?.message ?? "사전 서명 URL 생성 실패")
        return
      }

      const { presignedUrl, fileUrl, s3Key, expiresIn } = presigned.data as unknown as import("@/lib/types/image").CreatePresignedUrlSuccessResponse["data"];
      ...
      const saved = await saveImageMetadata(parsedMetadata.data)
