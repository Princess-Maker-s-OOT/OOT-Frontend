  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const { presignedUrl, fileUrl, s3Key } = presigned.data as any;
        ...
        if (saveResult.success && saveResult.data && saveResult.data.data) {
          const imageData = saveResult.data.data;
          if (imageData.id && imageData.url) {
            newImages.push({ id: imageData.id, url: imageData.url });
          }
        }
      }
      setUploadedImages((prev) => [
        ...prev,
        ...newImages.filter(img => img && img.id && img.url)
      ]);
    } catch (err: any) {
