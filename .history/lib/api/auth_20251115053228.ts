...
export async function exchangeOAuthToken(
  ...
  const response = await fetch(`${API_BASE_URL}/auth/oauth2/token/exchange`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      code,
...
export async function logout(): Promise<ApiResponse<void>> {
  ...
  const response = await fetch(`${API_BASE_URL}/auth/logout?deviceId=${deviceId}`, {
    method: "POST",
    headers: {
...
export async function getDeviceList(): Promise<ApiResponse<DeviceInfoResponse[]>> {
  ...
  const response = await fetch(`${API_BASE_URL}/auth/devices?currentDeviceId=${deviceId}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
...
export async function removeDevice(targetDeviceId: string): Promise<ApiResponse<void>> {
  ...
  const response = await fetch(
    `${API_BASE_URL}/auth/devices/${targetDeviceId}?currentDeviceId=${currentDeviceId}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${accessToken}`,
