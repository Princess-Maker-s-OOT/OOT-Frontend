  ...
  const fetchAllData = async (date?: string) => {
    ...
    try {
      ...
      if (userResult.success) {
        setUserStats(userResult.data ?? null)
      } else {
        console.error("사용자 통계 에러:", userResult)
        setError(prev => prev ? `${prev}, 사용자 통계: ${userResult.message}` : `사용자 통계: ${userResult.message}`)
      }

      if (clothesResult.success) {
        setClothesStats(clothesResult.data ?? null)
      } else {
        console.error("옷 통계 에러:", clothesResult)
        setError(prev => prev ? `${prev}, 옷 통계: ${clothesResult.message}` : `옷 통계: ${clothesResult.message}`)
      }

      if (salePostResult.success) {
        setSalePostStats(salePostResult.data ?? null)
      } else {
        console.error("판매글 통계 에러:", salePostResult)
        setError(prev => prev ? `${prev}, 판매글 통계: ${salePostResult.message}` : `판매글 통계: ${salePostResult.message}`)
      }

      if (categoryResult.success) {
        setCategoryStats(categoryResult.data ?? null)
      } else {
        console.error("카테고리 통계 에러:", categoryResult)
        setError(prev => prev ? `${prev}, 카테고리 통계: ${categoryResult.message}` : `카테고리 통계: ${categoryResult.message}`)
      }
    } catch (err) {
