...
import { getMyWearRecords } from "@/lib/api/wear-record"
...
export default function MyWearRecordList() {
  ...
  useEffect(() => {
    getMyWearRecords().then((res) => {
      if (res.data) setRecords(res.data.content)
      else setError(res.message ?? "오류가 발생했습니다.")
    })
  }, [])
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {records.map((record) => (
        <div key={record.wearRecordId} className="border rounded p-4 bg-white shadow-sm flex gap-4 items-center">
          {record.clothesImageUrl
            ? <img src={record.clothesImageUrl} alt="옷 이미지" className="w-20 h-20 object-cover rounded" />
            : <div className="w-20 h-20 flex items-center justify-center bg-gray-100 text-gray-400 rounded">이미지 없음</div>
          }
          <div>
