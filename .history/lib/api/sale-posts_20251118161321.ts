...
export async function getSalePosts(params: {
  ...
  if (params.distance !== undefined) queryParams.append("distance", params.distance.toString())
...
export async function getMySalePosts(params: {
  ...
  sort?: string
  direction?: "ASC" | "DESC"
}): Promise<ApiResult<SliceResponse<SalePostSummaryResponse>>> {
  const queryParams = new URLSearchParams()
  ...
  return apiGet<SliceResponse<SalePostSummaryResponse>>(`${API_BASE_URL}/api/v1/sale-posts/my?${queryParams.toString()}`)
}
