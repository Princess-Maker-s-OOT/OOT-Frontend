...
export async function apiClient(
  ...
  if (requiresAuth) {
    ...
    console.log("[apiClient] accessToken:", accessToken ? `${accessToken.substring(0, 20)}...` : "없음")
    if (accessToken) {
      requestHeaders["Authorization"] = `Bearer ${accessToken}`
    } else {
      console.warn("[apiClient] 인증 토큰이 없습니다. 로그인이 필요할 수 있습니다.")
    }
  }
...
export async function apiPost<T>(
  ...
  try {
    ...
    console.log("[apiPost] data BEFORE stringify:", data)
    const bodyString = data ? JSON.stringify(data) : undefined
    console.log("[apiPost] body AFTER stringify:", bodyString)
