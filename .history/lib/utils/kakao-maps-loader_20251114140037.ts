...
class KakaoMapsLoader {
  ...
  static getInstance(options?: KakaoMapsLoaderOptions): KakaoMapsLoader {
    if (!KakaoMapsLoader.instance && options) {
      ...
      try {
        ...
        script.onload = () => {
          ...
          setTimeout(() => {
            ...
            if (kakao?.maps) {
              // autoload=false인 경우 kakao.maps.load() 호출
              if (typeof kakao.maps.load === "function") {
                console.log("📞 Calling kakao.maps.load()...")
                kakao.maps.load(() => {
                  console.log("✅ Kakao Maps object initialized via load()")
                  this.state = "loaded"
                  resolve()
                })
              } else {
                // autoload=false가 아닌 경우 이미 로드됨
                console.log("✅ Kakao Maps object already initialized")
                this.state = "loaded"
                resolve()
              }
            } else {
  ...
  private _buildScriptUrl(): string {
    ...
    params.set("autoload", "false") // document.write 오류 방지
