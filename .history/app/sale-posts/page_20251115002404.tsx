  ...
  async function loadPosts() {
    try {
      ...
      const result = await apiGet<SalePostListResponse>(
        "/api/v1/sale-posts/public?page=0&size=20",
        { requiresAuth: false }
      );

      console.log("판매글 API 응답:", result);

      if (result.success && result.data) {
        console.log("판매글 목록:", result.data.content);
      ...
      {!loading && !error && posts.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {posts.map((p) => (
            ...
            >
              <img
                src={p.thumbnailUrl || "https://via.placeholder.com/300x200?text=No+Image"}
                alt={p.title}
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <span className={`text-xs px-2 py-1 rounded ${
                  ...
                  {p.status === "SELLING" ? "판매중" : p.status === "RESERVED" ? "예약중" : "판매완료"}
                </span>
              </div>
            </Link>
          ))}
