...
export default function ImageUploadForm() {
  ...
  const [type, setType] = useState<"user" | "clothes" | "closet" | "salepost">("closet")
  // 타입 매핑 함수
  const typeMap: Record<string, "USER" | "CLOTHES" | "CLOSET" | "SALEPOST"> = {
    user: "USER",
    clothes: "CLOTHES",
    closet: "CLOSET",
    salepost: "SALEPOST"
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      // 1. Presigned URL 생성 (소문자 ENUM)
      const presigned = await createPresignedUrl({
        fileName: file.name,
        type: type // "user" | "clothes" | "closet" | "salepost"
      })
    ...
    <form onSubmit={handleUpload} className="space-y-6 bg-white p-6 rounded shadow max-w-md mx-auto">
      ...
      <div>
        ...
        <select
          ...
          value={type}
          onChange={(e) => setType(e.target.value as "user" | "clothes" | "closet" | "salepost")}
        >
