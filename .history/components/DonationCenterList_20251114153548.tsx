  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      try {
        ...
        const kakaoLatLng = new windowKakao.maps.LatLng(userLocation.lat, userLocation.lng)

        if (!mapRef.current) {
          mapRef.current = new windowKakao.maps.Map(mapContainerRef.current, {
            center: kakaoLatLng,
            level: 5,
          })
        ...
        const bounds = new windowKakao.maps.LatLngBounds()

        centers.forEach((center) => {
          const position = new windowKakao.maps.LatLng(center.latitude, center.longitude)
          const marker = new windowKakao.maps.Marker({
            position,
            map: mapRef.current,
            title: center.name,
          })

          const infoWindow = new windowKakao.maps.InfoWindow({
            content: `
          ...
          windowKakao.maps.event.addListener(marker, "click", () => {
    ...
    // autoload=false 모드이므로 LatLng이 이미 로드되었는지 확인
    if (typeof windowKakao.maps.LatLng === "function") {
      // 이미 로드됨 - 직접 초기화
      console.log("✅ Kakao Maps API already loaded, initializing directly")
      initializeMap()
    } else {
      // 아직 로드 안됨 - kakao.maps.load() 사용 (한 번만 호출)
      console.log("🚀 Loading Kakao Maps API with kakao.maps.load()...")
