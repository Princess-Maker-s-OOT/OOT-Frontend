...
export async function uploadImageToS3(
  ...
  try {
    ...
    const presignedRequest: CreatePresignedUrlRequest = {
      fileName: file.name,
      type: directory === "profile" ? "user" : directory === "sale-post" ? "salepost" : directory,
    }

    const presignedResponse = await createPresignedUrl(presignedRequest)
    ...
    const { presignedUrl, imageUrl, imageId } = presignedResponse.data as any
    ...
    const metadataRequest: SaveImageMetadataRequest = {
      ...
      size: file.size,
      url: imageUrl,
      s3Key: (presignedResponse.data as any).s3Key,
      type:
        directory === "profile" ? "USER"
        : directory === "sale-post" ? "SALEPOST"
        : directory === "clothes" ? "CLOTHES"
        : directory === "closet" ? "CLOSET"
        : "CLOTHES",
    }

    const metadataResponse = await saveImageMetadata(metadataRequest)
